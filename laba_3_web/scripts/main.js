...
let newsDisplayed = NEWS_PER_PAGE;
...
function renderNews() {
    ...
    const newsToShow = sortedNews.slice(0, newsDisplayed);
    ...
    const btnLoadMoreNews = document.getElementById('btn-load-more-news');
    if (newsDisplayed >= sortedNews.length) {
        btnLoadMoreNews.classList.add('hidden');
    } else {
        btnLoadMoreNews.classList.remove('hidden');
    }
}

function initLoadMoreNews() {
    const btnLoadMoreNews = document.getElementById('btn-load-more-news');
    btnLoadMoreNews.addEventListener('click', () => {
        newsDisplayed += NEWS_PER_PAGE; 
        renderNews(); 
    });
}
...
function updateProducts(resetPagination = true) {
    ...
    if (checkedVendors.length > 0) filteredProducts = filteredProducts.filter(p => checkedVendors.includes(p.vendor));
    if (checkedFamilies.length > 0) filteredProducts = filteredProducts.filter(p => checkedFamilies.includes(p.family));
    if (checkedBrands.length > 0) filteredProducts = filteredProducts.filter(p => checkedBrands.includes(p.brand));
    if (selectedVram !== 'all') filteredProducts = filteredProducts.filter(p => p.vram === selectedVram);
